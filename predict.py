...
if __name__ == "__main__":
    ...
    model = torch.load('C:/Users/Siddhant Tiwari/Skin-Care/skin-model-pokemon.pt', map_location=torch.device('cpu'))

    # device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    device = torch.device('cpu')
    model.to(device)

    img = Image.open('C:/Users/Siddhant Tiwari/Skin-Care/acne-closed-comedo-2.jpg').convert("RGB")
